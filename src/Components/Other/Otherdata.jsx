

const Otherdata = () => {
    return (
        <div>
        <div className="flex flex-row justify-center items-center ">
              <h1 className=" text-4xl font-bold  text-blue-800 underline"> Appertment Facilities</h1>
              <img className="w-36" src="https://i.ibb.co/ZmZjjtn/lo.png" alt="" />
        </div>
      <div className="px-16 text-xl text-green-800">
              <p>Apartment amenities are the nonessential features or services provided to the residents of an apartment building. Examples in an apartment include gyms, pools, parking garages, laundry rooms, playgrounds, and dog parks.</p>
      </div>

       <div className="flex flex-row justify-center gap-8 md:mt-16 mb-24 data2  items-center bg-fixed  ">
            <div className="text-center text-white">
               <h1 className="mt-4 font-bold text-3xl ">Appertment Facilities</h1>
               <p className="mt-4 font-bold text-xl">Apartments & Flats for Sale in Bangladesh. Showing 1-25 of 4,836 ads. </p>
               <p className="font-bold">Ready Flat for sale at Gausul Azam Avenue, Uttara, Dhaka. Tk 10,500 per sqft.</p>
               <button className="btn mt-6 bg-red-600 text-white">Book The Rooms</button>       
          </div>
         
          <div className="">
              <img className="h-[400px] w-[450px]" src="https://i.ibb.co/1fb7mRv/la.jpg" alt="" />
          </div>
         
       </div>




      </div>
    );
};

export default Otherdata;